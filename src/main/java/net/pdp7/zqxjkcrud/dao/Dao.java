...
public class Dao {
  ...
  public Map<String, Table> getTables() {
    return getCatalog().getTables().stream()
  ...
  public void update(Update update) {
    for (TableUpdate tableUpdate : update.tableUpdates) {
      Map<Field<Object>, Object> fields =
          tableUpdate.fields.entrySet().stream()
              .collect(Collectors.toMap(e -> DSL.field((String) e.getKey()), e -> e.getValue()));
      switch (tableUpdate.action) {
        case INSERT:
          dslContext.insertInto(DSL.table(tableUpdate.table.getName())).set(fields).execute();
          break;
        case UPDATE:
          dslContext
              .update(DSL.table(tableUpdate.table.getName()))
              .set(fields)
